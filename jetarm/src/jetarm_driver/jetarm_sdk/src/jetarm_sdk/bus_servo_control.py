#!/usr/bin/env python3
# encoding: utf-8
# Date:2021/07/24
# 串口舵机代码控制例程(interface servo code control routine)
import rospy
import signal
from hiwonder_interfaces.srv import GetServoState 
from hiwonder_interfaces.msg import MultiRawIdPosDur, RawIdPosDur

def set_servos(pub, duration, pos_s):
    msg = MultiRawIdPosDur(id_pos_dur_list=list(map(lambda x: RawIdPosDur(int(x[0]), int(x[1]), int(duration)), pos_s)))
    pub.publish(msg)

if __name__ == '__main__':
    rospy.init_node('servo_control_example', anonymous=True)

    running = True
    def shutdown(signum, frame):
        global running

        running = False
        rospy.loginfo('shutdown')
        rospy.signal_shutdown('shutdown')

    signal.signal(signal.SIGINT, shutdown)

    # 发布舵机位置(publish servo position)
    joints_pub = rospy.Publisher('/controllers/multi_id_pos_dur', MultiRawIdPosDur, queue_size=1)
    while running:
        try:
            # 参数1:发布句柄(parameter1: publish handle)
            # 参数2:运行时间(ms)(parameter2: running times(ms))
            # 参数3:((舵机id, 舵机位置(脉宽)), (舵机id, 舵机位置(脉宽), ...)(parameter3: ((servo id, servo position(pulse-width)),(servo id, servo position(pulse-width), ...)
            #set_servos(joints_pub, 500, ((6, 460), (5, 633), (4, 500), (3, 410), (2, 666)))
            # 延时，等待舵机转到指定位置(delay, wait for servo rotate to the specified position)
            #rospy.sleep(0.5)
            
            set_servos(joints_pub, 1000, ((5, 400), ))
            rospy.sleep(0.5)
            
            set_servos(joints_pub, 1000, ((5, 600),))
            rospy.sleep(0.5)

            # 获取舵机当前电压(get servo current voltage)
            res = rospy.ServiceProxy('/servo_control/get_servo_state', GetServoState)('voltage', 5)
            print('voltage: ' + str(round(res.value[0]/1000.0, 1)) + 'V')
        except Exception as e:
            print(e)
            break
