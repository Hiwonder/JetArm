...
def get_area_max_contour(contours, threshold=100):
    ...
    for c in contours:  # 历遍所有轮廓(iterate through all contours)
        ...
        if contour_area_temp > contour_area_max:
            ...
            if contour_area_temp > threshold:  # 只有在面积大于50时，最大面积的轮廓才是有效的，以过滤干扰(only when the area is greater than 50, the contour with the largest area is considered valid to filter out interference)
                area_max_contour = c
...
def perspective_transform(img, src, dst, debug=False):
    """
    执行透视变换：将倾斜视角拍摄到的道路图像转换成鸟瞰图，即将摄像机的视角转换到和道路平行。(perform perspective transformation: transform the skewed view of the road captured by the camera into a bird's-eye view, aligning the camera's perspective with the road)
    :param img: 输入图像(input image)
    :param src: 源图像中待测矩形的四点坐标(the coordinates of the four points of the rectangle to be tested in the source image)
    :param dst: 目标图像中矩形的四点坐标(the coordinates of the four points of the rectangle in the target image)
    """
    img_size = (img.shape[1], img.shape[0])
    # 手动提取用于执行透视变换的顶点(manually extract vertices for performing perspective transformation)
    '''
    # left_down
    # left_up
    # right_up
    ...
    dst = np.float32(
        [[65, 430],
         ...
         [575,430]])
    '''
    m = cv2.getPerspectiveTransform(src, dst)  # 计算透视变换矩阵(compute the perspective transformation matrix)
    if debug:
    ...
    #unwarped = cv2.warpPerspective(warped, m_inv, (warped.shape[1], warped.shape[0]), flags=cv2.INTER_LINEAR)  # 调试(adjustment)
