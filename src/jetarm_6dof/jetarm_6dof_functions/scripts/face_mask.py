...
class FacemaskNode:
    def __init__(self):
        ...
        weights = rospy.get_param("~weights", "/home/hiwonder/weights/facemask_v5_160.trt") # 获取参数， 权重文件路径(get parameter, the path to the weight file)
    ...
    def image_callback(self, ros_image: Image):
        ...
        try:
            ...
            pass
        # 因为 pycuda 要求上下文建立的及执行要在同一个线程中, 如果不能在话题回调中执行识别， 要放到队列里在主线程执行识别(because PyCUDA requires the context to be created and execution to be in the same thread, if recognition cannot be performed in the topic callback, it needs to be put into a queue and executed in the main thread)
