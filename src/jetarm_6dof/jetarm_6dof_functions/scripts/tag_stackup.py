...
class TagStackup():
    ...
    def set_target(self):
        actions.go_home(self.joints_pub)
        self.get_endpoint()
        self.stackup_step = 0
    ...
    def done_callback(self, state, result):  # 动作执行完毕回调(callback function when the action is completed)
        ...
        if not result.result.complete:  # 如果在移动中被取消，需要回到初始位置(if canceled while in motion, return to the initial position)
            bus_servo_control.set_servos(self.joints_pub, 500, ((1, 500), ))
            rospy.sleep(1)
            actions.go_home(self.joints_pub)
        ...
        if self.finish_percent == 1:  # 如果完整的完成移动(if the motion is completed in full)
            if self.stackup_step == 1:  # 如果完成了夹取(if grasping is completed)
                self.stackup_step = 2
                goal = MoveGoal()
                goal.grasp.mode = 'place'
                goal.grasp.position.x = -0.005
                goal.grasp.position.y = 0.165
                goal.grasp.position.z = self.target_position[0][2] + 0.009
    ...
    def start_stackup(self, pose_t, pose_R):
        ...
        goal.grasp.position.z = pose_t[2] - 0.025
    ...
    def image_callback(self, ros_image: RosImage):
        ...
        if (self.stackup_step == 0 or self.stackup_step == 10) and self.K is not None and self.D is not None and self.hand2cam_tf_matrix is not None and self.endpoint is not None:
            ...
            if len(tags) > 0 and tags[0].tag_id != 100:
                ...
                pose_world = np.matmul(self.endpoint, pose_end)  # 转换到机械臂世界坐标(convert to the robot arm's world coordinates)
                ...
                if self.target_count > 30:
                    self.target_count = 0
                    if self.stackup_step == 0 and self.enable_stackup:
        ...
        if self.err_msg is not None:
            ...
            for i, m in enumerate(err_msg):
                cv2.putText(result_image, m, (10, 50 + (i * 30)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 7)
                cv2.putText(result_image, m, (10, 50 + (i * 30)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
