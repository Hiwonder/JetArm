...
# 通过深度图识别物体的外形进行分类(classify object shapes through depth map recognition)
...
class RgbDepthImageNode:
    ...
    def enter_srv_callback(self, _: TriggerRequest):
        ...
        with self.lock:
            ...
            sync = message_filters.ApproximateTimeSynchronizer([rgb_sub, depth_sub, info_sub], 3, 0.03)  # 同步时间戳, 时间允许有误差在0.03s(synchronize timestamps, allowing for a time error of up to 0.03 seconds)
