...
action_path = "/home/hiwonder/factory_utils/arm_pc"
    ...
    if os.path.exists(actNum) is True:
        if runningAction is False:
            ...
            while True:
                ...
                else:   # 运行完才退出(exit after running completion)
                    break
            runningAction = False
        ...
        if update_ok:
            if online_action_times == 0:
                ...
                if action_group_finish:
                    action_group_finish = False
                runAction(online_action_num)
                online_action_times -= 1    # 运行完成后，进入空载(after running completion, enter idle state)
                if online_action_times == 0:
                    online_action_times = -1
            else:
                # 空载(idle state)
...
def start_action_thread():
    ...
    th1.setDaemon(True)  # 设置为后台线程，这里默认是True(set as a background thread, default is True)
    th1.start()
