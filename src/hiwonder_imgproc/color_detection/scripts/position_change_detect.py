...
# @data:2023/01/31
# @author:aiden
# 判断位置是否发生位置变化(determine if there is a change in position)
import math
import numpy as np
...
def position_change_or_not(last_point, current_points, distance):
    # 将上一次的某点与当前所有点对比，检测是否有距离符合设定的点，即当作位置没有改变的点 (compare a certain point from the previous time with all current points to detect if there are points whose distance matches the set point, which will be considered as points with unchanged positions)
...
def position_reorder(current_points, last_points, distance=10):
    ...
    # 对比上一次和当前点的位置，如果位置没有改变，则相应的颜色标签不变，否则从1开始重新安排标签序号(compare the positions of the previous and current points. If the position has not changed, keep the corresponding color label unchanged; otherwise, start rearranging the label numbers from 1)
    new_points = []
    haved_change_points = []
    for p in last_points:  # 对上一次的所有点和当前点进行位置对比(compare the positions of all points from the previous time with the current points)
