...
class CircleMic:
    ...
    # 麦克风阵列切换(switch circular microphone array)
    ...
    # 唤醒词更换（浅定制）(wake-up word replacement(shallow customization))
    def set_wakeup_word(self, str_pinyin="xiao3 huan4 xiao3 huan4"):
        # 参数为中文拼音(the parameters are in Chinese Pinyin)
    ...
    # 数据串口发送(data serial port transmission)
    def send_data(self, header, args):
        ...
        self.serialHandle.write(packet)  # 发送主控消息(send controller message)

    # 发送数据(send data)
    def send(self, header, args):
        self.serialHandle.write([0xa5, 0x01, 0x01, 0x04, 0x00, 0x00, 0x00, 0xa5, 0x00, 0x00, 0x00, 0xb0])  # 发送握手请求(send hand-shaking request)
        while True:
            result = None
            ...
            if recv_data == header_[0]:
                ...
                if recv_data == header_[1]:
                    ...
                    else:  # 没有收到确认(the confirmation has not been received)
                        recv_data = self.serialHandle.read(4)
                        self.serialHandle.read((recv_data[1] << 8 | recv_data[0]) + 1)
                        
                        time.sleep(0.1)
                        self.serialHandle.write([0xa5, 0x01, 0x01, 0x04, 0x00, 0x00, 0x00, 0xa5, 0x00, 0x00, 0x00, 0xb0])  # 继续发送发送握手请求(continue to send handshake request)
    ...
    # 检测是否唤醒以及唤醒对应角度(detect whether it is awakened and the corresponding angle after awakening)
                        ...
                        if m is not None:
                            ...
                            if m is not None:
                                ...
                                if self.flag_pub is not None and self.angle_pub is not None:
                                    ...
                                    angle = self.val_map(angle, 0, 360, 360, 0) + 240  # 和圆形兼容(compatible with circles)
                                    if angle >= 360:
                                        angle -= 360
                                    msg = Int32()
                                    msg.data = int(angle)
...
class AwakeNode:
    ...
    # 设置唤醒词服务(set a wake-up word service)
